...
function woocommerce_gateway_payhere_init() {
	...
	class WC_Gateway_PayHere extends WC_Payment_Gateway {
		...
		public function __construct(){
			...
			if ($this->settings['enable_sub_cancel'] == 'yes'){
				$supports_array[]		= 'subscription_cancellation'; // todo
			}
		...
		function init_form_fields(){
			...
			$this->form_fields = array(
				...
				// Enable Subscriptions Cancellation
				'enable_sub_cancel' => array(
					'title'			=> __('Subscription Cancellations', 'woo_payhere'),
					'type'			=> 'checkbox',
					'label'			=> __('Enable Subscription Cancellations', 'woo_payhere'),
					'default'		=> 'no',
					'description'	=> __('Automatically cancel the PayHere Subscription when cancelling a Subscription from WooCommerce', 'woo_payhere'),
					'desc_tip'		=> true
				),
				// PayHere Business App ID
				'ph_app_id'	=> array(
					'title'			=> __('PayHere Business App ID', 'woo_payhere'),
					'type'			=> 'text',
					'description'	=> __('App ID for the Business App created in your PayHere Account'),
					'desc_tip'		=> true,
					'class'			=> 'ph_app_credentials'
				),
				// PayHere Business App Secret
				'ph_app_secret'		=> array(
					'title'			=> __('PayHere Business App Secret', 'woo_payhere'),
					'type'			=> 'text',
					'description'	=> __('App Secret for the Business App created in your PayHere Account'),
					'desc_tip'		=> true,
					'class'			=> 'ph_app_credentials'
				),
		...
		function generate_payhere_form($order_id){
			...
			$should_process_as_subscription = false;
			$subscription_err = null;
			$this->process_as_subscription_if_needed(
				$payhere_args, $should_process_as_subscription,
				$subscription_err,
				$order, 
				$effective_merchant_secret);
					...
					try{
						...
						if ($verification_required){
							...
							if ($effective_merchant_secret) {
								...
								if (($md5hash != $md5sig) || $_REQUEST['payhere_amount'] != ($order -> order_total)) {
									$verified = false;
								}
							}  
						}
		...
		private function process_as_subscription_if_needed(&$payhere_args, &$process_status, &$subscription_err, $order, $effective_merchant_secret) {
			...
			printf(
				'
				<pre>
					should_process_as_subscription = %s
					is_synchronized_subscription = %s
					price_per_period = %s
					sign_up_fee = %s
					billing_period = %s
					billing_interval = %s
					trial_period = %s
					length_from_timestamp = %s (%s)
					time_end = %s (%s)

					timestamp_next_payment = %s (%s)
					timestamp_trial_end = %s (%s)
					timestamp_start = %s (%s)
					billing_length = %s
				...
				',
				json_encode($process_status),
				json_encode($is_synced_subscription),
				json_encode($sub_price_per_period),
				json_encode($sub_sign_up_fee),
				json_encode($sub_billing_period),
				json_encode($sub_billing_interval),
				json_encode($sub_trial_period),
				json_encode($length_from_timestamp),
				json_encode($this->convert_timestamp_readable($length_from_timestamp)),
				json_encode($subscription->get_time( 'end' )),
				json_encode($this->convert_timestamp_readable($subscription->get_time('end'))),
				json_encode($next_payment_timestamp),
				json_encode($this->convert_timestamp_readable($next_payment_timestamp)),
				json_encode($trial_end_timestamp),
				json_encode($this->convert_timestamp_readable($trial_end_timestamp)),
				json_encode($start_timestamp),
				json_encode($this->convert_timestamp_readable($start_timestamp)),
				json_encode($sub_billing_length),
				json_encode($sub_trial_length)
			);*/

			// Guard Errors

			if ($sub_billing_period != $sub_trial_period){
